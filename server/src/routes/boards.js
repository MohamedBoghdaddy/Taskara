...
router.post(
  ...
  guard(async (req, res) => {
    ...
    if (req.app.get("io"))
      req.app.get("io").to(`board:${req.params.id}`).emit("column:added", col);
...
router.delete(
  ...
  guard(async (req, res) => {
    ...
    board.cards.forEach((c) => {
      if (c.columnId === colId) c.columnId = fallback || "todo";
    });
...
router.post(
  ...
  guard(async (req, res) => {
    ...
    const board = await Board.findById(req.params.id);
    if (!board) return res.status(404).json({ error: "Board not found" });
    const cardsInCol = board.cards.filter(
...
router.patch(
  ...
  guard(async (req, res) => {
    ...
    if (req.app.get("io"))
      req.app.get("io").to(`board:${req.params.id}`).emit("card:updated", card);
    res.json(card);
  }),
);
  ...
  guard(async (req, res) => {
    ...
    if (req.app.get("io"))
      req.app
        .get("io")
        .to(`board:${req.params.id}`)
        .emit("card:deleted", req.params.cardId);
    res.json({ success: true });
  }),
);
