...
export default function WebhooksPage() {
  ...
  const load = () => webhooksApi.getWebhooks().then(d => setWebhooks(d.webhooks || [])).catch(() => toast.error('Failed to load webhooks')).finally(() => setLoading(false));
  useEffect(() => { load(); }, []);
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '860px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '700', marginBottom: '4px' }}>🔗 Webhooks</h1>
          <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Receive real-time events in your external systems via HTTP POST.</p>
        </div>
        <Button variant="primary" onClick={() => setShowModal(true)}>+ New Webhook</Button>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-muted)', border: '1px dashed var(--border)', borderRadius: 'var(--radius)' }}>
          ...
          <p style={{ fontSize: '12px', marginTop: '8px', opacity: 0.7 }}>All POST requests include <code>X-Taskara-Signature</code> for verification.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {webhooks.map(wh => (
            <div key={wh._id} style={{ padding: '16px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '3px' }}>
                    <code style={{ fontSize: '13px', fontWeight: '600', color: 'var(--text-primary)', background: 'var(--surface-alt)', padding: '2px 7px', borderRadius: '4px' }}>{wh.url}</code>
                    <span style={{ fontSize: '11px', padding: '2px 7px', borderRadius: '99px', background: wh.active ? '#16a34a22' : 'var(--surface-alt)', color: wh.active ? 'var(--success)' : 'var(--text-muted)' }}>
                      {wh.active ? 'Active' : 'Paused'}
                    </span>
                  </div>
                  {wh.description && <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{wh.description}</div>}
                ...
                <div style={{ display: 'flex', gap: '6px', flexShrink: 0, marginLeft: '12px' }}>
                  <Button size="sm" variant="secondary" onClick={() => handleTest(wh)} disabled={testing === wh._id}>{testing === wh._id ? 'Sending…' : 'Test'}</Button>
                  <Button size="sm" variant="secondary" onClick={() => handleToggle(wh)}>{wh.active ? 'Pause' : 'Enable'}</Button>
                  <Button size="sm" variant="danger" onClick={() => handleDelete(wh._id)}>Delete</Button>
                </div>
              </div>
        ...
        <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {ALL_EVENTS.map(ev => (
                <button key={ev} type="button" onClick={() => toggleEvent(ev)} style={{ padding: '4px 10px', borderRadius: 'var(--radius)', border: `1px solid ${form.events.includes(ev) ? 'var(--primary)' : 'var(--border)'}`, background: form.events.includes(ev) ? 'var(--primary)18' : 'var(--surface)', color: form.events.includes(ev) ? 'var(--primary)' : 'var(--text-secondary)', fontSize: '12px', cursor: 'pointer' }}>
                  {ev}
                </button>
              ))}
            </div>
