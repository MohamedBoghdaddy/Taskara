...
const tone = {
  queued: { bg: "#e2e8f0", color: "#000000" },
  ready: { bg: "#dbeafe", color: "#000000" },
  awaiting_approval: { bg: "#fef3c7", color: "#000000" },
  scheduled: { bg: "#dcfce7", color: "#000000" },
  in_progress: { bg: "#ede9fe", color: "#000000" },
  blocked: { bg: "#fee2e2", color: "#000000" },
  completed: { bg: "#dcfce7", color: "#000000" },
  cancelled: { bg: "#e2e8f0", color: "#000000" },
  paused: { bg: "#fef3c7", color: "#000000" },
  failed: { bg: "#fee2e2", color: "#000000" },
};
...
const hasScheduledFollowUp = (item) =>
  Boolean(
    ...
    (item?.actionLogs || []).some((entry) => entry.status === "scheduled"),
  );
...
const FEEDBACK_OPTIONS = [
  ...
  {
    key: "should_have_required_approval",
    label: "Should have required approval",
  },
];
...
const getSafetyTone = (level = "medium") =>
  level === "high"
    ? { bg: "#fff1f2", color: "#000000", border: "rgba(244,63,94,0.18)" }
    : level === "medium"
      ? { bg: "#fff7ed", color: "#000000", border: "rgba(251,146,60,0.20)" }
      : { bg: "#ecfdf5", color: "#000000", border: "rgba(16,185,129,0.18)" };

const getNextSafetyAction = (item) => {
  const relevant =
    (item?.actionLogs || []).find((entry) =>
      ["awaiting_approval", "scheduled", "pending", "failed"].includes(
        entry.status,
      ),
    ) ||
    [...(item?.actionLogs || [])]
...
const buildTypicalUsage = ({
  audienceKey,
  audienceInfo,
  planState,
  integrationCoverage = [],
}) => {
  const readyConnectors = integrationCoverage
    .filter((entry) => entry.writebackReady)
    .map((entry) => formatLabel(entry.provider));
      ...
      >
        ...
        <div
          style={{
            fontSize: "12px",
            color:
              percent >= 90 ? "#b91c1c" : percent >= 75 ? "#b45309" : "#0f766e",
            fontWeight: 800,
          }}
        >
...
function ActionButton({ label, onClick, disabled, variant = "secondary" }) {
  const palette =
    variant === "primary"
      ...
      : variant === "danger"
        ? { bg: "#fff1f2", color: "#000000", border: "rgba(244,63,94,0.18)" }
        : { bg: "#ffffff", color: "#000000", border: "rgba(148,163,184,0.18)" };
...
function OnboardingPanel({
  ...
  const stepTone = (step) =>
    onboarding.currentStep === step
      ...
      : onboarding.currentStep > step
        ? { bg: "#dcfce7", border: "rgba(34,197,94,0.18)", color: "#000000" }
        : { bg: "#ffffff", border: "rgba(148,163,184,0.18)", color: "#000000" };
    ...
    >
      ...
      >
        <div>
          ...
          >
            Taskara will walk through source, action, result, and approval using
            the live workflow engine. Demo mode stays available when the
            required connector is not ready yet.
          </div>
        </div>
        <div
          style={{
            ...
            padding: "16px 18px",
            borderRadius: "18px",
            background: onboarding.integrationReady ? "#ecfdf5" : "#fff7ed",
            border: `1px solid ${onboarding.integrationReady ? "rgba(16,185,129,0.18)" : "rgba(251,146,60,0.18)"}`,
          }}
        >
          <div
            style={{
          ...
          <div style={{ fontSize: "13px", color: "#000000", lineHeight: 1.7 }}>
            {onboarding.integrationReady
              ...
              : onboarding.demoAvailable
                ...
                : onboarding.integrationStatus?.details?.[0] ||
                  "This integration still needs setup."}
          </div>
        </div>
      </div>
        ...
        {[1, 2, 3, 4, 5, 6].map((step) => {
          ...
          return (
            ...
            >
              ...
              <div
                style={{ fontSize: "13px", fontWeight: 700, lineHeight: 1.5 }}
              >
                {labels[step]}
              </div>
            </div>
      ...
      <div style={{ display: "grid", gap: "16px" }}>
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          ...
          />
          <ActionButton
            label={saving ? "Preparing..." : "Run guided demo"}
            variant="primary"
            onClick={onRunDemo}
            disabled={saving}
          />
        ...
        >
          ...
          <div style={{ fontSize: "13px", color: "#000000", lineHeight: 1.8 }}>
            {onboarding.resultSummary ||
              "Choose a workflow type, connect one required integration, or start the guided demo."}
          </div>
          ...
          {onboarding.currentStep >= 5 ? (
            ...
            >
              Estimated manual time removed: about{" "}
              {onboarding.savedMinutesEstimate || 0} minutes on the first run.
            </div>
          ) : null}
        </div>
      </div>
...
function WorkflowItemCard({
  ...
  const safetyTone = getSafetyTone(
    nextSafetyAction?.riskLevel || item.riskLevel || "medium",
  );
    ...
    >
      ...
      >
        <div>
          ...
          >
            {item.description || item.sourceContext?.excerpt}
          </div>
        </div>
        <div
          style={{
      ...
      >
        ...
        <div
          style={{
            padding: "14px",
            borderRadius: "16px",
            background: safetyTone.bg,
            border: `1px solid ${safetyTone.border}`,
          }}
        >
          <div
            style={{
          ...
          >
            ...
            >
              {normalizeConfidenceScore(
                nextSafetyAction?.confidenceScore ?? item.confidenceScore ?? 0,
              )}
              /100
            </div>
            <div
          ...
          <div style={{ fontSize: "12px", color: "#000000", lineHeight: 1.6 }}>
            {(nextSafetyAction?.riskReasons || item.safetyReasons || [])
              .slice(0, 2)
              .join(" ")}
          </div>
        </div>
        ...
        >
          ...
          >
            {item.traceability?.outcomeSummary ||
              "Outcome trace will appear here after execution."}
          </div>
        </div>
        <div
          style={{
      ...
      >
        ...
        {hasScheduledFollowUp(item) ? (
          <ActionButton label="Stop follow-up" onClick={onStopFollowUp} />
        ) : null}
        {(item.actionLogs || []).some(
      ...
      >
        ...
        <div
          style={{
            ...
            gap: "8px",
            flexWrap: "wrap",
            marginBottom: feedbackMode === "incorrect" ? "10px" : 0,
          }}
        >
          <ActionButton
              ...
              {FEEDBACK_OPTIONS.map((option) => (
                <button
                  ...
                  style={{
                    ...
                    background: feedbackCategories.includes(option.key)
                      ? "#fff1f2"
                      : "#ffffff",
                    color: feedbackCategories.includes(option.key)
                      ? "#be123c"
                      : "#334155",
                    fontWeight: 700,
                    fontSize: "12px",
...
export default function DashboardPage() {
  ...
  const refresh = useCallback(
    async (nextAudience = audience) => {
      ...
      try {
        ...
        toast.error(
          error.response?.data?.error || "Failed to load execution hub",
        );
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    if (
      ...
      onboarding.audienceType !== audience
    ) {
  ...
  const handleIngest = async () => {
    ...
    const payload = {
      ...
      sourceType:
        form.sourceType || dashboard?.audience?.readsFirst || "manual",
      title:
        form.title ||
        fallback?.title ||
        `${AUDIENCE_CONTENT[audience].label} workflow`,
    ...
    try {
      ...
      toast.success(
        result.duplicate
          ? "This source was already ingested."
          : "Workflow input ingested and routed.",
      );
      setForm((current) => ({ ...current, title: "", text: "" }));
      await refresh(audience);
    } catch (error) {
      toast.error(
        error.response?.data?.error || "Failed to ingest workflow input",
      );
    } finally {
      setSaving(false);
    }
  ...
  const blockers = [
    ...(dashboard?.integrationCoverage || [])
      ...
      .map((entry) => ({
        ...
        message:
          entry.details?.[0] || "Writeback is not ready for this workflow.",
      })),
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error ||
          "Failed to update onboarding workflow type",
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleRunDemo = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error || "Failed to run guided onboarding",
      );
    } finally {
      setSaving(false);
    }
    ...
    try {
      ...
      toast.success(
        payload.verdict === "correct"
          ? "Feedback saved as correct"
          : "Feedback saved for review",
      );
      await refresh(audience);
    } catch (error) {
      toast.error(
        error.response?.data?.error || "Failed to save workflow feedback",
      );
      throw error;
    } finally {
      setSaving(false);
      ...
      <div style={{ maxWidth: "1220px", margin: "0 auto" }}>
        ...
        >
          ...
          >
            ...
            <div
              style={{ fontSize: "13px", color: "#000000", lineHeight: 1.7 }}
            >
              Approval-sensitive external actions are guarded automatically.
              Assignment reasons and audit logs stay visible on every item.
            </div>
          </div>
        </div>
          ...
          {AUDIENCE_LIST.map((entry) => (
            <button
              ...
              style={{
                ...
                border: `1px solid ${audience === entry.key ? "rgba(15,118,110,0.18)" : "rgba(148,163,184,0.18)"}`,
                background:
                  audience === entry.key ? "rgba(15,118,110,0.10)" : "#ffffff",
                color: audience === entry.key ? "#0f766e" : "#334155",
                fontWeight: 800,
                cursor: "pointer",
              }}
        ...
        {loading || !dashboard ? (
          ...
          >
            Loading workflow execution data...
          </div>
        ) : (
          <>
            <OnboardingPanel
            ...
            >
              <StatCard
                ...
                value={dashboard.summary.activeCount}
                detail="Execution items currently moving through the workflow."
                icon={<WorkflowIcon size="sm" color="#0f766e" />}
              />
              <StatCard
                label="Pending approvals"
                value={dashboard.summary.pendingApprovals}
                detail="Risky external actions waiting for operator approval."
                icon={<ShieldIcon size="sm" color="#0f766e" />}
              />
              <StatCard
                label="Completed this week"
                value={dashboard.summary.completedThisWeek}
                detail="Workflow items that reached a finished state in the last 7 days."
                icon={<CheckCircleIcon size="sm" color="#0f766e" />}
              />
              <StatCard
                label="Scheduled follow-ups"
                value={dashboard.summary.scheduledFollowUps}
                detail="Active sequences with timing controls and stop conditions."
                icon={<ClockIcon size="sm" color="#0f766e" />}
              />
            </div>
            ...
            {planState ? (
              <div
                style={{
                  display: "grid",
                  gridTemplateColumns:
                    "minmax(0, 1.2fr) repeat(auto-fit, minmax(210px, 1fr))",
                  gap: "14px",
                  marginBottom: "24px",
                }}
              >
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        Pricing is based on workflows executed, actions
                        executed, and integrations connected. Upgrade when usage
                        or automation depth outgrows the current package.
                      </div>
                    </div>
                    <ActionButton
                      label="See pricing"
                ...
                <UsageMeter
                  ...
                  usage={planState.usage?.workflowsExecuted}
                />
                <UsageMeter
                  label="Actions / month"
                  usage={planState.usage?.actionsExecuted}
                />
                <UsageMeter
                  label="Integrations"
                  usage={planState.usage?.integrationsConnected}
                />
              </div>
            ) : null}
              ...
              >
                ...
                <div style={{ display: "grid", gap: "14px" }}>
                  ...
                  >
                    ...
                    >
                      {(dashboard.audience.sourceTypes || ["manual"]).map(
                        (source) => (
                  ...
                  >
                    ...
                    />
                    Execute safe actions immediately and stop for approvals when
                    risk is detected.
                  </label>
                  ...
                  >
                    ...
                    >
                      Taskara will preserve source context, group related work,
                      prevent duplicates where possible, and attach a visible
                      audit trail.
                    </div>
                    <ActionButton
                      label={saving ? "Working..." : "Ingest and run"}
                      variant="primary"
                    ...
                    {dashboard.integrationCoverage.map((entry) => (
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            style={{
                              fontSize: "12px",
                              fontWeight: 800,
                              color: entry.connected ? "#0f766e" : "#b45309",
                            }}
                          >
                        ...
                        >
                          ...
                          {entry.ready ? "Writeback ready" : "Needs attention"}
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            >
              {(dashboard.audience.metrics || []).map((metric) => (
                <MetricCard
                  key={metric.id}
                  label={metric.label}
                  value={dashboard.metrics?.[metric.id] ?? 0}
                />
              ))}
            </div>
            ...
            >
              <div style={{ display: "grid", gap: "18px" }}>
                {(dashboard.items || []).length === 0 ? (
                  ...
                  >
                    No execution items yet for this audience. Paste a real
                    operational source above and Taskara will create structured
                    execution work with approvals, audit logs, and sync intent.
                  </div>
                ) : (
                  dashboard.items.map((item) => (
                    <WorkflowItemCard
                      ...
                      onApprove={() =>
                        runItemAction(
                          () => approveWorkflowItem(item._id, "approve"),
                          "Approval granted",
                        )
                      }
                      onReject={() =>
                        runItemAction(
                          () => approveWorkflowItem(item._id, "reject"),
                          "Approval rejected",
                        )
                      }
                      onPause={() =>
                        runItemAction(
                          () => controlWorkflowItem(item._id, "pause"),
                          "Workflow paused",
                        )
                      }
                      onResume={() =>
                        runItemAction(
                          () => controlWorkflowItem(item._id, "resume"),
                          "Workflow resumed",
                        )
                      }
                      onCancel={() =>
                        runItemAction(
                          () => controlWorkflowItem(item._id, "cancel"),
                          "Workflow cancelled",
                        )
                      }
                      onRun={() =>
                        runItemAction(
                          () => executeWorkflowItem(item._id),
                          "Ready actions executed",
                        )
                      }
                      onClaim={() =>
                        runItemAction(
                          () => assignWorkflowItem(item._id, user?._id),
                          "You now own this workflow item",
                        )
                      }
                      onStopFollowUp={() =>
                        runItemAction(
                          () => controlWorkflowItem(item._id, "stop_followup"),
                          "Follow-up cadence stopped",
                        )
                      }
                      onUndo={() =>
                        runItemAction(
                          () =>
                            controlWorkflowItem(item._id, "undo_last_action"),
                          "Last action reset for review",
                        )
                      }
                      onFeedback={(payload) =>
                        handleFeedback(item._id, payload)
                      }
                    />
                  ))
                )}
                ...
                >
                  ...
                  <div
                    style={{
                      marginBottom: "12px",
                      fontSize: "13px",
                      color: dashboard.migrationPreview?.validation
                        ?.readyToImport
                        ? "#0f766e"
                        : "#b45309",
                    }}
                  ...
                  {(dashboard.migrationPreview?.validation?.warnings || [])
                    .length ? (
                    <div
                      style={{
                        display: "grid",
                    ...
                    >
                      {dashboard.migrationPreview.validation.warnings.map(
                        (warning) => (
                          <div
                            key={warning}
                            style={{
                  ...
                  <div style={{ display: "grid", gap: "10px" }}>
                    {(dashboard.migrationPreview?.mappingPreview || []).map(
                      (entry) => (
                        <div
                    ...
                    {(dashboard.migrationPreview?.mappingPreview || [])
                      .length === 0 ? (
                      <div style={{ color: "#000000", lineHeight: 1.7 }}>
                        No migration mappings are loaded for this audience yet.
                      </div>
                    ) : null}
                  </div>
                </div>
                  ...
                  <div style={{ display: "grid", gap: "10px" }}>
                    {(dashboard.runs || []).map((run) => (
                      ...
                      >
                        ...
                        >
                          {run.input?.title || audienceInfo.label}
                        </div>
                        <div
                          style={{
                            fontSize: "13px",
                        ...
                        >
                          ...
                          {run.extractionSummary?.itemCount || 0} item(s)
                        </div>
                      </div>
                    ))}
                  </div>
