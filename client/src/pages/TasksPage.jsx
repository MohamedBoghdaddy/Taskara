...
// Map frontend types to backend taskType enum
const TYPE_TO_TASKTYPE = {
  feature: "general",
  bug: "bug",
  chore: "general",
  improvement: "general",
...
const STATUS_ICONS = {
  ...
  in_progress: { Icon: PlayIcon, color: "#000000" },
...
export default function TasksPage() {
  ...
  const loadTasks = async () => {
    setLoading(true);
    try {
      const d = await getTasks(filters);
      setTasks(d.tasks || []);
    } catch {
  ...
  const handleCreate = async (e) => {
    ...
    // Map frontend type to backend taskType
    const payload = {
      ...
      taskType: form.type
        ? TYPE_TO_TASKTYPE[form.type] || "general"
        : "general",
    ...
    try {
      ...
      // Verify response
      if (!task?._id) {
        throw new Error("Invalid server response: missing task ID");
      }
      ...
      console.error("Task creation error:", { payload, error });
    }
  };
          ...
          <Tooltip content="List view" placement="bottom">
            <button
              ...
              style={{
                ...
                gap: "5px",
                background:
                  view === "list" ? "var(--primary)" : "var(--surface)",
          ...
          <Tooltip content="Board view" placement="bottom">
            <button
              ...
              style={{
                ...
                gap: "5px",
                background:
                  view === "board" ? "var(--primary)" : "var(--surface)",
              ...
              {STATUSES.map((status) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {STATUS_ICONS[status] &&
                        React.createElement(STATUS_ICONS[status].Icon, {
                          size: "xs",
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      background: active ? `${def.color}18` : "var(--surface)",
...
function TaskListRow({ task, onStatusChange, onDelete, onClick }) {
  ...
  return (
    ...
    >
      ...
      {subtaskCount > 0 && (
        <Tooltip
          content={`${subtaskCount} subtask${subtaskCount > 1 ? "s" : ""}`}
          placement="top"
        >
          <span
            style={{
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        ...
        <div>
          ...
          >
            Description
            {showDesc ? (
              <EyeIcon size="xs" color="var(--text-muted)" />
            ) : (
              <EyeOffIcon size="xs" color="var(--text-muted)" />
            )}
          </label>
          {showDesc && (
            <textarea
