  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", gap: "14px", alignItems: "center", marginBottom: "16px", flexWrap: "wrap" }}>
        <div>
          <div style={{ fontSize: "12px", color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 700, marginBottom: "6px" }}>
            {audience.label}
          </div>
          <div style={{ fontSize: "24px", fontWeight: 800, letterSpacing: "-0.04em", color: "var(--text-primary)" }}>
            {audience.hero}
          </div>
        </div>
        <div style={{ padding: "10px 14px", borderRadius: "999px", background: "rgba(15,118,110,0.10)", border: "1px solid rgba(15,118,110,0.16)", color: "var(--text-primary)", fontWeight: 800, fontSize: "12px" }}>
          {audience.measuredResults.join(" | ")}
        </div>
      </div>

      {(Object.keys(metrics || {}).length === 0) ? (
        <div style={{ padding: "18px", borderRadius: "18px", background: "#f8fafc", border: "1px solid rgba(148,163,184,0.16)", color: "var(--text-secondary)", lineHeight: 1.7 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px" }}>
          {Object.entries(metrics || {}).map(([key, value]) => (
            <div key={key} style={{ padding: "18px", borderRadius: "18px", background: "#f8fafc", border: "1px solid rgba(148,163,184,0.16)" }}>
              <div style={{ fontSize: "12px", color: "var(--text-secondary)", textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 700, marginBottom: "8px" }}>
                {key.replace(/_/g, " ")}
              </div>
              <div style={{ fontSize: "30px", fontWeight: 900, letterSpacing: "-0.05em", color: "var(--text-primary)" }}>{value}</div>
            </div>
          ))}
        </div>
      )}
    ...
    >
      <div style={{ maxWidth: "1180px", margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: "16px", alignItems: "flex-start", flexWrap: "wrap", marginBottom: "24px" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "8px 14px", borderRadius: "999px", background: "rgba(15,118,110,0.10)", color: "#000000", border: "1px solid rgba(15,118,110,0.16)", fontSize: "12px", fontWeight: 800, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: "14px" }}>
            ...
            <p style={{ maxWidth: "880px", fontSize: "16px", lineHeight: 1.8, color: "#000000", margin: 0 }}>
          ...
          <div style={{ padding: "20px", borderRadius: "24px", background: "rgba(255,255,255,0.92)", border: "1px solid rgba(148,163,184,0.18)", boxShadow: "0 18px 60px rgba(15,23,42,0.06)", maxWidth: "320px" }}>
            <div style={{ display: "grid", gap: "10px" }}>
              {[
                ...
                <div key={item.text} style={{ display: "flex", alignItems: "flex-start", gap: "10px", color: "#000000", lineHeight: 1.7 }}>
        ...
        {loading ? (
          <div style={{ padding: "60px", textAlign: "center", color: "var(--text-secondary)" }}>Loading workflow analytics...</div>
        ) : (
