...
export default function CanvasPage() {
  ...
  const redraw = useCallback(() => {
    ...
    for (const p of paths) {
      if (p.type === 'pen' || p.type === 'eraser') {
        ...
        ctx.beginPath();
        ctx.strokeStyle = p.type === 'eraser' ? getComputedStyle(document.documentElement).getPropertyValue('--surface') || '#fff' : p.color;
  ...
  const ToolBtn = ({ t, icon, title }) => (
    <button title={title} onClick={() => setTool(t)} style={{ padding: '7px 10px', border: 'none', borderRadius: 'var(--radius)', background: tool === t ? 'var(--primary)' : 'var(--surface-alt)', color: tool === t ? '#fff' : 'var(--text-secondary)', cursor: 'pointer', fontSize: '15px' }}>
      {icon}
    </button>
  );
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* Toolbar */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderBottom: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0, flexWrap: 'wrap' }}>
        <span style={{ fontWeight: '700', fontSize: '14px', marginRight: '4px' }}>Canvas</span>
        <div style={{ display: 'flex', gap: '3px' }}>
          <ToolBtn t={TOOLS.SELECT}  icon="↖" title="Select" />
          <ToolBtn t={TOOLS.PEN}     icon="✏️" title="Pen" />
          <ToolBtn t={TOOLS.ERASER}  icon="⌫" title="Eraser" />
          <ToolBtn t={TOOLS.RECT}    icon="⬜" title="Rectangle" />
          <ToolBtn t={TOOLS.CIRCLE}  icon="⭕" title="Circle/Ellipse" />
          <ToolBtn t={TOOLS.ARROW}   icon="↗" title="Arrow" />
          <ToolBtn t={TOOLS.TEXT}    icon="T" title="Text" />
        </div>
        {/* Colors */}
        <div style={{ display: 'flex', gap: '3px', marginLeft: '4px' }}>
          {COLORS.map(c => (
            <button key={c} onClick={() => setColor(c)} style={{ width: 20, height: 20, borderRadius: '50%', background: c, border: color === c ? '2px solid var(--text-primary)' : '2px solid transparent', cursor: 'pointer', padding: 0, flexShrink: 0 }} />
          ))}
        </div>
        {/* Sizes */}
        <div style={{ display: 'flex', gap: '3px', alignItems: 'center' }}>
          {SIZES.map(s => (
            <button key={s} onClick={() => setSize(s)} style={{ padding: '4px 8px', border: 'none', borderRadius: 'var(--radius)', background: size === s ? 'var(--primary)' : 'var(--surface-alt)', color: size === s ? '#fff' : 'var(--text-secondary)', cursor: 'pointer', fontSize: '11px' }}>{s}</button>
          ))}
        </div>
        {/* Actions */}
        <div style={{ display: 'flex', gap: '4px', marginLeft: 'auto' }}>
          <button onClick={undo}      title="Undo (Ctrl+Z)"  style={btnStyle}>↩ Undo</button>
          <button onClick={redo}      title="Redo (Ctrl+Y)"  style={btnStyle}>↪ Redo</button>
          <button onClick={clear}     style={{ ...btnStyle, color: 'var(--error)' }}>🗑 Clear</button>
          <button onClick={saveImage} style={{ ...btnStyle, background: 'var(--primary)', color: '#fff' }}>⬇ Save PNG</button>
        </div>
      </div>
        ...
        {textInput && (
          <div style={{ position: 'absolute', left: textInput.x, top: textInput.y - 16 }}>
            <input
              ...
              onBlur={addText}
              style={{ background: 'rgba(255,255,255,0.9)', border: '1px dashed var(--primary)', borderRadius: '3px', padding: '2px 6px', fontSize: '16px', color: color, outline: 'none', minWidth: '80px' }}
